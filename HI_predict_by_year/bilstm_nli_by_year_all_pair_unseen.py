...
if __name__ == '__main__':
    ...
    unseen_data_path = "../VHID_all_pair_unseen/VHID_all_pair.tsv"
    ...
    # vocab_size, embedding_dim, hidden_size, dropout, embedding_matrix, output_dim
    BiLSTM_NLI_model = BiLSTM_NLI(len(kmer_dict.vocab), 768, 100, 0.1, embedding_matrix, 1)
    BiLSTM_NLI_model.to(device)
    # CNNmodel.embedding.weight.data.copy_(torch.from_numpy(embedding_matrix.detach.numpy()))
    loss_fn = nn.BCELoss()
    ...
    pd.DataFrame(predict_rsult).to_csv('all_pair_unseen_predict.csv')
    print(val_loss)
