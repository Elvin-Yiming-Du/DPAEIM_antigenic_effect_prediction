...
from CNN_NLI_Model import CNN_NLI
import random
import numpy as np
import torch
import csv
...
def train(train_loader, CNN_NLI_model):
    CNN_NLI_model.train()
    epochs = 51
    f1 = 0.0
    acc = 0.0
    ...
    for epoch in range(1, epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            predictions = CNN_NLI_model(features1, features2).squeeze(1)

            answer = nn.Sigmoid()(predictions)
            labels = batch[1].to("cpu")
            answer = answer.to("cpu")
            loss = loss_fn(answer, labels)
            ...
            n_correct += torch.where(binary_answer == labels, ones_tensor, zero_tensor).sum().item()
...
def val(validation_loader, CNN_NLI_model):
    CNN_NLI_model.eval()
    true_correct = 0
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(validation_loader):
            ...
            pre = CNN_NLI_model(features1, features2).squeeze(1)
...
if __name__ == '__main__':
    ...
    CNN_NLI_model = CNN_NLI(len(kmer_dict.vocab), 768, 1, 2, 1, 0.1, embedding_matrix, 100)
    CNN_NLI_model.to(device)
    loss_fn = nn.BCELoss()
    loss_fn = loss_fn.to(device)

    loss_fn = nn.BCELoss()
    loss_fn = loss_fn.to(device)
    optimizer = torch.optim.Adam(CNN_NLI_model.parameters(), lr=0.0001)
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[100, 200, 250], gamma=0.333,
                                                     last_epoch=-1)

    train(train_loader, CNN_NLI_model)
    val_loss = val(validation_loader, CNN_NLI_model)
    print(val_loss)
