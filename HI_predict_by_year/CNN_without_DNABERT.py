...
from sklearn.metrics import precision_recall_curve, average_precision_score, roc_curve, auc, precision_score, \
    recall_score, f1_score, confusion_matrix, accuracy_score, classification_report
...
def train(train_loader, CNNmodel):
    CNNmodel.train()
    epochs = 51
    f1 = 0.0
    acc = 0.0
    ...
    for epoch in range(1, epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            predictions = CNNmodel(features).squeeze(1)
        ...
        if epoch % 100 == 0 or epoch == epochs - 1:
            ...
            print("-----------------------------------")
            # print(f'| Epoch: {epoch:02} | Train Loss: {eveloss/train_loader.__len__():.3f}')

def val(validation_loader, CNNmodel):
    CNNmodel.eval()
    true_correct = 0
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(validation_loader):
            true_correct += batch[1].sum().item()
            features = batch[0].reshape(batch_size, -1)
            pre = CNNmodel(features).squeeze(1)
            ...
            # n_correct += (torch.max(pre, 1)[1].view(batch[1].size()) == batch[1]).sum().item()
            n_total += batch_size
            n_loss += loss.item()
...
if __name__ == '__main__':
    ...
    CNNmodel = CNN(len(kmer_dict.vocab), 768, 1, 2, 1, 0.1)
    CNNmodel.embedding.weight.data.copy_(embedding_matrix)
    CNNmodel.to(device)
    ...
    train(train_loader, CNNmodel)
    val_loss = val(validation_loader, CNNmodel)
    print(val_loss)
